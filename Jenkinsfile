    ...
    stages {
        ...
        stage('Install Playwright') {
            steps {
                script {
                    bat '''
                        docker run --rm -v "%WORKSPACE%:/workspace/test_playwright" -w /workspace mcr.microsoft.com/playwright:v1.31.2-jammy /bin/bash -c "npm install @playwright/test @playwright/test@1.44.1"
                    '''
                }
            }
        }
